...
**Voice (first pass).** `src/lib/house-style.ts` holds the quality floor and a thin voice contract. The floor is structural — mechanism, actual/consensus/prior, a named instrument, a falsifier, 800–1,200 words, no headline restatement, no CTA-only watchlist — not a Bloomberg/FXStreet pastiche. `VOICE_SAMPLES` is empty until desk samples land; drop them there to tighten phrasing. `findStyleViolations()` flags banned phrases, template/watchlist headings, lede-as-headline, CTA watchlists, and stubs under 800 words. Drafts that fail that floor get one bounded follow-up call.
