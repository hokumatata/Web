...
**Voice.** `src/lib/house-style.ts` holds `QUALITY_FLOOR` and `VOICE_SAMPLES`. The samples are original house paragraphs that encode the desk-note register (price → move → two pressures → tension; 3–5 claim heads; mechanism chain; path + kill) and a tighter feature register — not reprints. The floor is 800–1,200 words, no headline restatement, no CTA-only watchlist. `findStyleViolations()` flags banned phrases, template/watchlist headings, lede-as-headline, CTA watchlists, fewer than three claim heads, and stubs under 800 words. Drafts that fail that floor get one bounded follow-up call.
