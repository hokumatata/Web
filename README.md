...
**Voice.** `src/lib/house-style.ts` holds the shared voice contract, derived from a study of live FXStreet coverage (see `NEWSROOM_STYLE.md`). It bans the tells rather than prescribing a template: no essay-label headings (`Introduction`, `Market Context`, `Analysis`, `Market Takeaway`), a banned-phrase list, varied sentence and paragraph rhythm, inline wire-style attribution, and depth via mechanism, second-order effects, the counter-case and what would change the picture — not padding. `findStyleViolations()` checks these deterministically on every draft (banned phrases, template headings, missing sections, stub length) and the breaches show up as review flags. Drafts that come back too short get one bounded follow-up call to develop the analysis.
